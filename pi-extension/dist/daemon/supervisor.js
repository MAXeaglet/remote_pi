...
export function resolvePiBinOption(piBin) {
    ...
    try {
        const which = _execFileSync(process.platform === "win32" ? "where" : "which", ["omp"], { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] });
...
export class Supervisor {
    ...
    async _opReply(id) {
        ...
        try {
            require("node:fs").appendFileSync(require("node:path").join(require("node:os").homedir(), ".omp", "remote", "supervisord-debug.log"), `[${new Date().toISOString()}] reply: getMessages() on ${id}\n`);
        }
        catch { }
        ...
        try {
            require("node:fs").appendFileSync(require("node:path").join(require("node:os").homedir(), ".omp", "remote", "supervisord-debug.log"), `[${new Date().toISOString()}] reply: got ${messages.length} messages\n`);
        }
        catch { }
