 ...
 *  writes never touch the developer's real `~/.omp/remote/daemons.json`. */
let testHome: string;
...
describe("registryPath", () => {
  test("honors REMOTE_PI_HOME env override", () => {
    expect(registryPath()).toBe(join(testHome, ".omp", "remote", "daemons.json"));
  });
});
...
describe("loadRegistry / saveRegistry", () => {
  ...
  test("malformed JSON tolerated (returns empty)", () => {
    ...
    mkdirSync(join(testHome, ".omp", "remote"), { recursive: true });
